...
import { Card } from "../StudyCards/CardsContext";
...
export default function Home() {
  ...
  useEffect(() => {
    const keyDownHandler = (event: KeyboardEvent) => {
      if (totalTexts <= 0) {
        // should not do anything if there are no texts
        return;
      }
  ...
  useEffect(() => {
    if (
      typedWords[gameIndex] &&
      activeWord.length > 0 &&
      index < typedWords[gameIndex].length
    ) {
  ...
  function getWord(
    ...
    const isCorrect =
      word === correctWord || idx >= index || gameidx != gameIndex
        ? ""
        : "incorrect-word";
    ...
    return (
      ...
      >
        ...
        {isActive && (
          <div className={gameStart ? "cursor-no-blink" : "cursor"}>
  ...
  function getTextComponent(textData: { title: string; text: string }[]) {
    return (
      <div className="flex flex-col gap-[20px]">
        {textData &&
          textData.map(({ title, text }, index) => (
            <div className="flex flex-col" key={index}>
              <div className="font-raleway font-medium text-2xl w-fit">
                {title}
              </div>
              <div className="flex flex-wrap gap-[10px] font-robotomono text-2xl">
                {typedWords[index] &&
                  typedWords[index].map((wordData, idx) => {
                    return getWord(
                      wordData.word,
